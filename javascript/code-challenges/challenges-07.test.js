...
const sortStarWarsCharacters = (newArr) => {
  return newArr.sort((a, b) => parseInt(b.height) - parseInt(a.height));
};
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.map((item) => {
    const ingredients = item.slice(item.indexOf(' ', item.indexOf(' ') + 1) + 1);
