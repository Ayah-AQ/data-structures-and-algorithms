...
const count = (target, input) => {
  return input.reduce((count, array) => {
    const occurrences = array.filter(element => element === target).length;
...
const totalSum = (input) => {
  return input.reduce((totalSum, array) => {
    const sum = array.reduce((arraySum, element) => arraySum + element, 0);
    return totalSum + sum;
  }, 0);
};
/* ------------------------------------------------------------------------------------------------
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(array =>
    array
      .filter(element => typeof element === 'number' && element % 5 === 0)
...
let findShortest = (data) => {
  const shortestCharacter = data
    .filter(character => character.height !== "unknown")
